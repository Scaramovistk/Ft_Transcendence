...
from match.serializers import MatchHistorySerializer
...
class VerifyOTPSerializer(serializers.Serializer):
    ...
    def create(self, validated_data: dict):
        ...
        user.save(update_fields=["login_otp_used", "last_login"])
        return {
            "refresh": str(refresh),
...
class AccountDetailSerializer(serializers.ModelSerializer):
    ...
    def get_match_history(self, instance):
        try:
            player: Player = instance.player
        except Player.DoesNotExist:
            return {}
        serializer = MatchHistorySerializer(instance=player)
